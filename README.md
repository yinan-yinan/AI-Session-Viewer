...
- 主要导航为所有项目、搜索会话、收藏与使用统计；Skills、无效项管理、回收站、Provider 同步与 Oh My Pi 模型供应商配置集中到侧栏底部的「工具与管理」。节点选择仍在来源下方。
...
- **列表虚拟化**：项目和会话的列表模式单列，网格模式按可用宽度自适应 1~4 列，均保留行级虚拟化，仅渲染可见行；加载时显示扫描进度条
...
- **会话导出**：点击会话卡片「操作 → 导出会话」，选 JSON / Markdown / HTML 任一格式保存；桌面端走系统保存框，Web 端浏览器下载
...
- 首屏固定加载末尾 30 条；向上滚动或点击按钮时加载更早消息并保持滚动位置，不因视口较高自动扩张首屏 DOM
- 超过 24 条消息时仅立即渲染最近 18 条，较早消息在接近可视区时再解析 Markdown 和代码高亮
- 文件监听按实际变更路径刷新；其他会话写入不会重复读取或重绘当前消息
- 桌面端项目与会话扫描在后台线程执行，启动时不重复深扫 rollout，避免列表刷新阻塞消息读取和窗口交互
- 会话账单在消息首屏完成后只扫描当前会话文件，并合并重复请求，不再因头部账单徽标遍历全部 Codex 历史
...
在消息详情页中，每条用户提问下方常显“Fork 分叉”按钮，提问折叠时也可直接操作。鼠标悬停提问时还会显示终端续聊和收藏按钮。缺少分叉定位信息时，Fork 按钮禁用并提示刷新会话或更新应用后重试。
...
**Fork**：Claude、Codex、Grok 和 Oh My Pi 均支持从指定提问处分叉。新会话保留此前历史和该轮完整回复（包括工具调用及结果），不包含下一条提问及后续对话；原会话保持不变。提问汇总页也提供“从此处分叉”按钮。OMP 会沿所选节点的祖先链保留历史，排除其他分支，并复制会话附件。
...
Grok 保留完整原始聊天记录及提示词上下文，重新生成会话 ID、摘要计数和请求标识。缺失祖先、损坏的中间记录或失效的分叉定位会报错，不会退化成仅复制可见文本。

Web API：`POST /api/sessions/fork`，JSON 请求为 `{ source, originalFilePath, userMsgUuid }`，返回 `{ newSessionId, newFilePath, projectPath, projectId }`。接口复用现有认证及各来源的会话路径校验；Codex/Grok 的消息标识包含原始行定位及内容摘要，不依赖当前分页位置。
...
- **会话级账单徽标**：消息详情页「详情」菜单中显示本会话累计 cost + 请求次数，点击查看逐请求账单，「复制 Markdown」一键导出表格
