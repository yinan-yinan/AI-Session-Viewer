...
- 桌面与 Web 均支持 Claude、Codex、Grok、Oh My Pi 四来源 Fork：从指定提问创建独立会话，保留此前历史与该轮完整回复，原会话保持不变；消息详情和提问汇总均提供分叉入口。
- Web 与远程节点分叉后直接打开新会话；Claude、Codex 和 Oh My Pi 可沿用应用内续聊，Grok 提供服务器端续聊命令。
- 显示设置新增侧栏 Agent 显隐选项，至少保留一个可见来源；最近浏览按节点保存，并按当前来源展示。

### Changed

- 项目与会话默认采用便利贴式卡片网格，按可用宽度排列 1～4 列，保留列表切换和行级虚拟化；会话卡片操作集中到常显菜单。
- 侧栏保留四个 Agent 纵向入口，将次要工具收进「工具与管理」；窄屏采用抽屉导航。
- 精简消息阅读区域，以「显示」「详情」菜单管理次要操作；提问目录和续聊输入框默认收起并记住偏好，Fork 按钮常显。
- 统一页面标题、间距、明暗主题和键盘焦点样式；统计页突出核心数字，逐请求账单使用独立横向滚动区域。

### Fixed

- 修复快速切换来源后打开相同路径会话时可能出现消息空白的问题。
- 修复窄屏布局初始化与窗口缩放时导航可交互状态可能不同步的问题。

### Version

- 将工作区版本统一提升到 `2.22.0`，同步 `package.json`、`package-lock.json`、根 `Cargo.lock`、`src-tauri/tauri.conf.json` 与 3 个 Cargo manifest。
...
- 修复切换会话数据源时短暂显示旧项目计数或“0 个项目”的问题；切换动作现在立即进入明确的加载状态，快速来回切换也会复用进行中的请求并保持加载提示。
- 修复项目列表命中会话缓存时可能把空会话或损坏会话计入数量的问题；Claude、Codex、Grok 现在统一只统计有效会话。
- 修复 Codex 仅含元数据、没有可见活动的 rollout 被当作有效会话，导致主列表出现大量“（无标题）”条目和项目会话数虚高的问题；可见性摘要现在进入持久化文件索引，未变化的 rollout 不再因打开项目而重复扫描。
- 修复 Grok 项目列表与会话列表重复解析全部历史文件及空会话计入项目数量的问题；缓存现在按会话目录记录修改签名并持久化，普通加载和文件监听只重读发生变化的会话。
...
- MCP 声明支持 Claude JSON 与 Codex TOML 跨机同步，默认剔除 Token、密码、认证头和凭据路径；覆盖时保留目标机已有敏感值，并校验预览哈希。
- 插件与 marketplace 安装声明支持跨机差异预览，不传输插件 cache、安装目录或机器绝对路径。
